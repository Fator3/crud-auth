<template>
  <v-dialog v-model="details.show" max-width="600" style="position:static !important;" persistent>
    <v-card>
      <v-card-title class="headline">
        {{ details.action == 'multiedit' ? $t('global.details.multipleUpdateTitle') : title }}
      </v-card-title>
      <v-form v-on:submit.prevent v-model="details.formValid">
        <v-card-text class="details-list">
          <div v-for="(field, i) in fields" :key="i">
            <v-layout row wrap>
              <v-flex v-if="details.action == 'multiedit' && field.show" class="sm1">
        ...
        <v-card-actions>
          ...
          <v-btn :disabled="!details.formValid" v-if="details.action == 'create'" color="green" text @click="store()">{{
          ...
          >
          <v-btn v-else-if="details.action == 'multiedit'" color="orange" text @click="updateSelected()">{{
            $t('global.details.buttons.modifySelected')
          }}</v-btn>
        </v-card-actions>
      </v-form>
    </v-card>
...
export default {
  ...
  methods: {
    ...
    store () {
      this.storeItem([this.itemData, this.$t('global.alerts.stored'), this.$t('global.alerts.storeError')]).then(
        (response) => {
          this.close()
