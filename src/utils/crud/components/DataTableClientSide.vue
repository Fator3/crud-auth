  ...
  <v-card flat>
    ...
    >
      <template slot="center">
        <!-- Search by fields -->
        <v-menu offset-y :close-on-content-click="false" max-height="50vh" v-if="fieldFilters">
          <template v-slot:activator="{ on }">
            <v-btn large color="grey" icon v-on="on">
        ...
        <!-- Search in table -->
        <span v-if="mainFilter" class="data-table__search">
          <v-text-field
    ...
    >
      <template v-for="(header, i) in headers" v-slot:[`item.${header.value}`]="{ item }">
        <span :key="i">
          ...
          />
          <span v-else>
            <data-table-row-field :value="item[header.value]" :text-mode="textMode(item, header.value)" />
          </span>
        </span>
      </template>
